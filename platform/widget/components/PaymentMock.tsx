...
const PaymentMock: React.FC<PaymentMockProps> = ({ language, service, booking }) => {
  const { texts, clinicId, clinic } = useConfig();
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (startTimeIso && endTimeIso) {
        // Use configured URL or derive from API_URL
        // Default to same origin /api/reserve-slot if not set
        const apiBase = import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/create-session', '') : '/api';
        const reserveApiUrl = import.meta.env.VITE_RESERVE_SLOT_URL || `${apiBase}/reserve-slot`;
        ...
        try {
          const reserveResponse = await fetch(reserveApiUrl, {
            ...
            body: JSON.stringify({
              ...
              customer_phone: booking.patientData.phone,
          ...
          // If it's a slot-taken error, re-throw it so user knows
          if (reserveError.message && (
            reserveError.message.includes('longer available') ||
            reserveError.message.includes('nav pieejams') ||
            reserveError.message.includes('недоступно')
          )) {
            throw reserveError;
          }
          // Otherwise log warning but allow payment to proceed (soft fail)
          console.warn('[SlotLock] Reservation warning:', reserveError);
        }
      }

      // 4. Call our secure server-side Stripe session endpoint
      // The Stripe secret key is stored securely in Cloudflare environment variables
      console.log('[PaymentMock] Creating Stripe session via secure server endpoint...');

      const fullName = `${booking.patientData.firstName} ${booking.patientData.lastName}`;

      // Build metadata object for the session
      const metadata = {
        booking_date: isoDate,
        booking_time: booking.selectedTime || undefined,
        service_id: service.id,
        serviceName: service.name[booking.language] || service.name[Language.EN],
        language: booking.language,
        duration: service.durationMinutes ? String(service.durationMinutes) : undefined,
        doctor_id: booking.selectedSpecialist?.id || undefined,
        doctor_name: booking.selectedSpecialist?.name || undefined,
        pending_booking_id: pendingBookingId || undefined,
        clinic_id: clinicId,
        clinic_email: clinic.clinicEmail || undefined,
        customer_name: fullName,
        customer_email: booking.patientData.email,
        customer_phone: booking.patientData.phone || undefined
      };
      ...
      const response = await fetch('/api/create-stripe-session', {
        ...
        body: JSON.stringify({
          ...
          service_name: service.name[Language.EN] || 'Dental Service',
          success_url: successUrl,
          cancel_url: cancelUrl,
          customer_email: booking.patientData.email,
          currency: clinic.settings?.currency === 'USD' ? 'usd' : 'eur',
          metadata
        })
      });
    ...
    <div className="animate-fade-in max-w-md mx-auto pb-28 sm:pb-0">
      ...
      <div className="bg-gray-50 dark:bg-slate-800/50 rounded-xl p-5 mb-6">
        ...
        <div className="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-slate-700">
          ...
          <span className="text-sm font-medium text-gray-900 dark:text-white">{clinic.settings?.currency === 'USD' ? '$' : '€'}{service.price}</span>
        </div>
        ...
        <div className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-slate-700">
          ...
          <span className="text-sm font-medium text-gray-900 dark:text-white">{clinic.settings?.currency === 'USD' ? '$' : '€'}{remainingBalance}</span>
        </div>
        ...
        <div className="flex justify-between items-center pt-4">
          ...
          <span className="text-2xl font-bold text-primary">{clinic.settings?.currency === 'USD' ? '$' : '€'}{depositAmount}</span>
        </div>
      </div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>{texts.paySecure[language]} • {clinic.settings?.currency === 'USD' ? '$' : '€'}{depositAmount}</span>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                  </svg>
                </>
